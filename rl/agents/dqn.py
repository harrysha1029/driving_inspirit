...
from tensorflow.keras.layers import Dense, Input, Lambda, Layer, Permute, Reshape
...
class DQNAgent(AbstractDQNAgent):
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        trainable_model.compile(
            optimizer=optimizer, loss=losses, metrics=combined_metrics
        )
    ...
    def metrics_names(self):
        ...
        model_metrics = [
            name.replace(dummy_output_name + "_", "") for name in model_metrics
        ]
...
class NAFLayer(Layer):
    ...
    def call(self, x, mask=None):
        ...
        if self.mode == "full":
            ...
            if K.backend() == "theano":
                ...
                P = tf.scan(
                    fn, L_flat, initializer=K.zeros((self.nb_actions, self.nb_actions))
                )
            else:
                raise RuntimeError('Unknown Keras backend "{}".'.format(K.backend()))
    ...
    def compute_output_shape(self, input_shape):
        ...
        if input_shape[0][1] != expected_elements:
            raise RuntimeError(
                "Input 0 (L) should have {} elements but has {}".format(
                    input_shape[0][1]
                )
            )
        if input_shape[1][1] != self.nb_actions:
            raise RuntimeError(
                "Input 1 (mu) should have {} elements but has {}".format(
                    self.nb_actions, input_shape[1][1]
                )
            )
        if input_shape[2][1] != self.nb_actions:
            raise RuntimeError(
                "Input 2 (action) should have {} elements but has {}".format(
                    self.nb_actions, input_shape[1][1]
                )
            )
...
class NAFAgent(AbstractDQNAgent):
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        os_in = [
            Input(shape=shape, name="observation_input_{}".format(idx))
        ...
        combined_out = Lambda(lambda x: x[0] + x[1], output_shape=lambda x: x[0])(
            [A_out, V_out]
        )
