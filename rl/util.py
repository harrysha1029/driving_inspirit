...
from tensorflow.keras.models import Model, Sequential, model_from_config
...
def get_soft_target_model_updates(target, source, tau):
    target_weights = target.trainable_weights + sum(
        [l.non_trainable_weights for l in target.layers], []
    )
    source_weights = source.trainable_weights + sum(
        [l.non_trainable_weights for l in source.layers], []
    )
    ...
    for tw, sw in zip(target_weights, source_weights):
        updates.append((tw, tau * sw + (1.0 - tau) * tw))
    return updates
...
class WhiteningNormalizer(object):
    ...
    def update(self, x):
        ...
        self.std = np.sqrt(
            np.maximum(
                np.square(self.eps),
                self._sumsq / float(self._count) - np.square(self.mean),
            )
        )
