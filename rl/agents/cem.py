...
class CEMAgent(Agent):
    ...
    def select_action(self, state, stochastic=False):
        ...
        if stochastic or self.training:
            return np.random.choice(
                np.arange(self.nb_actions), p=np.exp(action) / np.sum(np.exp(action))
            )
    ...
    def update_theta(self, theta):
        if theta is not None:
            ...
            ), "Invalid theta, shape is {0} but should be {1}".format(
                theta.shape, self.theta.shape
            )
            assert not np.isnan(theta).any(), "Invalid theta, NaN encountered"
            assert (
                theta[self.num_weights :] >= 0.0
            ).all(), "Invalid theta, standard deviations must be nonnegative"
            self.theta = theta
        else:
    ...
    def backward(self, reward, terminal):
        ...
        if terminal:
            ...
            if (
                self.step > self.nb_steps_warmup
                and self.episode % self.train_interval == 0
            ):
