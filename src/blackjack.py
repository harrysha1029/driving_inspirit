...
def play_blackjack_verbose(player=human_blackjack_player):
    env = gym.make("Blackjack-v0")
    result = run(env, player, True)[-1][-1]
    if result > 0:
...
def q_learn(env, eps, gamma, alpha, epochs):
    ...
    for _ in tqdm(range(epochs)):
        history = run(env, lambda obs: eps_policy(obs, q_table, eps))
